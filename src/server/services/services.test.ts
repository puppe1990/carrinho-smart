...
describe('cart service', () => {
  it('returns a full overview for the active cart', () => {
    const overview = getCartOverview(repo, STORE_ID)
  ...
  it('flags extra items that were not planned on the list', () => {
    const cart = repo.carts.getActive(STORE_ID)!
  ...
  it('updates budget, quantities, additions and removals', () => {
    ...
    expect(setBudget(repo, cartId, 50000).budgetCents).toBe(50000)

    const added = addLine(repo, cartId, {
    ...
    const updated = updateLineQuantity(repo, added.line.id, 5)
    expect(updated.line.quantity).toBe(5)

    const afterRemove = removeLine(repo, added.line.id)
    expect(afterRemove.kindCount).toBe(overview.summary.kindCount)

    expect(clearCart(repo, cartId).kindCount).toBe(0)
  })

  it('checks out the cart into a purchase', () => {
    const overview = getCartOverview(repo, STORE_ID)
    const purchase = checkout(repo, overview.cart.id)
    expect(purchase.id).toBeTruthy()
    expect(purchase.totalCents).toBe(overview.summary.totalCents)
    expect(repo.carts.get(overview.cart.id)?.status).toBe('checked_out')
  })
})
...
describe('scanner service', () => {
  it('looks up a known barcode and suggests the current price', () => {
    ...
    const result = lookupBarcode(repo, product.barcode, STORE_ID)
  ...
  it('returns null for unknown barcodes', () => {
    expect(lookupBarcode(repo, '0000000000000', STORE_ID)).toBeNull()
  })
...
describe('list service', () => {
  it('returns the list, items and progress', () => {
    const overview = getShoppingListOverview(repo)
    expect(overview.list?.id).toBe('list-current')
  ...
  it('adds a quick item to the list', () => {
    const overview = getShoppingListOverview(repo)
    const created = addQuickItem(repo, overview.list!.id, '  Guardanapos  ')
    expect(created.name).toBe('Guardanapos')
    const refreshed = getShoppingListOverview(repo)
  ...
  it('scans a list item into the cart and marks it as scanned', () => {
    ...
    const result = scanListItem(repo, pending.id, {
      cartId: cart.id,
      unitPriceCents: 1234,
      quantity: 1,
    })
    ...
    expect(getShoppingListOverview(repo).progress.scannedCount).toBe(
...
describe('history service', () => {
  it('aggregates purchases for a month', () => {
    ...
    const history = getHistory(repo, year, month)
  ...
  it('builds a purchase summary with category distribution', () => {
    const purchase = repo.purchases.list()[0]
    const summary = getPurchaseSummary(repo, purchase.id)!
  ...
  it('returns null for an unknown purchase', () => {
    expect(getPurchaseSummary(repo, 'nope')).toBeNull()
  })
})
